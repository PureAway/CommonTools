...
public class FlutterConsoleFilter implements Filter {
            ...
            try {
                ...
                handler.addProcessListener(new ProcessAdapter() {
                    ...
                    public void processTerminated(@NotNull final ProcessEvent event) {
                        Utils.showErrorNotification(project, "Error Opening " + myPath);
                    }
                });
                handler.startNotify();
            } catch (ExecutionException e) {
                Utils.showErrorNotification(project, "Error Opening External File Exception: " + e.getMessage());
            }
        }
    }
    ...
    public Filter.Result applyFilter(final String line, final int entireLength) {
        ...
        if (file != null) {
            // "open ios/Runner.xcworkspace"
            final boolean openAsExternalFile = Utils.isXcodeFileName(pathPart);
