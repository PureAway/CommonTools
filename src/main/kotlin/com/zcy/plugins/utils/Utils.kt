...
object Utils {
    ...
    fun generateExportPath(dirPath: String?) {
        val pubSpecFile = File(dirPath, "pubspec.yaml")
        if (!pubSpecFile.exists()) {
            showInfo(
                "can not found pubspec.yaml file at current selected directory."
        ...
        val inputStream = FileInputStream(pubSpecFile)
        val yaml = Yaml()
        val map = yaml.load<HashMap<String, String>>(inputStream)
        val name = map["name"]
        if (name.isNullOrEmpty()) {
            showInfo(
                "can not found name at pubspec.yaml file."
            )
            return
        }
        val libraryFile = File(dirPath, "lib/${name}.dart")
        if (!libraryFile.exists()) {
            showInfo(
                "can not found file which named ${libraryFile.absolutePath}."
            )
            return
        }

        val stringBuilder = StringBuilder("\n\n")
        val codeFile = File(dirPath, "lib")
        buildExportPath(stringBuilder, codeFile, codeFile.absolutePath, name)

        val fileWriter = FileWriter(libraryFile.absolutePath, true)
        val bw = BufferedWriter(fileWriter)
        bw.write(stringBuilder.toString())
        bw.close()
        fileWriter.close()
    ...
    private fun buildExportPath(build: StringBuilder, file: File, dirPath: String, name: String) {
        val files = file.listFiles()
        if (!files.isNullOrEmpty()) {
            files.forEach {
                if (it.isDirectory) {
                    buildExportPath(build, it, dirPath, name)
                } else {
                    if (it.name.endsWith(".dart", true)
                        && !it.name.endsWith("$name.dart", true)
                        && !it.name.endsWith(".g.dart", true)
                        && !it.name.endsWith(".path.dart", true)
                        && !it.name.endsWith(".export.dart", true)
                    ) {
                        val absolutePath = it.absolutePath
                        val resultPath = absolutePath.split(dirPath)[1]
                        build.append("export \'package:$name$resultPath\';")
                            .append("\n")
                    }
                }
            }
    ...
    fun buildRunnerRebuild(
        ...
        if (toolWindow != null) {
            ...
            project.asyncTask(commandName, runAction = {
                ...
                line.addParameters(listOf("pub", "run", "build_runner", "build", "--delete-conflicting-outputs"))
